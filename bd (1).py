...
data = json.load(open("Study_1_full_peaks.json",'rb'))
groups = data['groups']
...
def corrected_graph(compound,frag,sample):
    ...
    a1=list(c[frag][sample][0])
    b1=list(c[frag][sample][1])
...
def set_display_children(compound,frags,samples,mode,alters):
    dt=[]
    dtc=[]
    r=0
    r1=0
    ...
    for frag in frags:
        ...
        for f in samples:
            f1=f.split("-")
            if f not in prep_data[compound][frag].keys():
                ints=t=[0]*len(prep_data[compound][frag][list(prep_data[compound][frag].keys())[0]][0])
                o=list(prep_data[compound][frag][list(prep_data[compound][frag].keys())[0]][1])
                state="rejected"
                area=0
                cor_area=0
                parent='0'
                ...
                t=list(prep_data[compound][frag][f][0])
                o=list(prep_data[compound][frag][f][1])
                for i in range(len(alters)):
                    if alters[i]=="SC":
                        t=scale_peak(t)
                    elif alters[i]=="AL":
                        print("inside aligner")
                        if r==0:
                                r=1
                ...
                for i in range(len(alters)):
                    if alters[i]=="SC":
                        ...
                        if r1==0:
                            r1=1
                            max_peak_rt=o[ints.index(max(ints))]
                        else:
                            max_rt_loc=o[ints.index(max(ints))]
                            for j in range(len(o)):
                                o[j]=o[j]-max_rt_loc+max_peak_rt
            dt.append(go.Scatter(
                        ...
                        y = t,
                        mode = 'lines',
                        name = frag+"_"+f1[len(f1)-1]
                        ))
            dtc.append(go.Scatter(
                        x = o,
                        y = list(ints),
                        mode = 'lines',
                        name = frag+"_"+f1[len(f1)-1]+"+"+state
                        ))
